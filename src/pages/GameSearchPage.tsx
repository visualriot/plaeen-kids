...
import { doc, getDoc, updateDoc, arrayUnion, arrayRemove, collection, addDoc, Timestamp } from 'firebase/firestore';
import { useAuthState } from 'react-firebase-hooks/auth';
import { Search, Filter, Gamepad2, Star, Plus, Check, Info, Sparkles, Monitor, Smartphone, Cpu, X, Heart, ExternalLink, ChevronDown, Shield } from 'lucide-react';
...
export const GameSearchPage = () => {
  ...
  const exploreGames = useCallback(async (pageNum = 1) => {
    ...
    try {
      ...
      try {
        ...
        const defaultGames = [
          { id: '1', name: 'Elden Ring', description: 'Rise, Tarnished, and be led by grace to brandish the power of the Elden Ring.', platforms: ['PC', 'PS5', 'Xbox'], genres: ['RPG'], image: 'https://picsum.photos/seed/elden/600/400', rating: 96, releaseDate: '2022-02-25', isChildFriendly: false },
          { id: '2', name: 'Valorant', description: 'A 5v5 character-based tactical shooter.', platforms: ['PC'], genres: ['Shooter'], image: 'https://picsum.photos/seed/valorant/600/400', rating: 80, releaseDate: '2020-06-02', isChildFriendly: false },
          { id: '3', name: 'Minecraft', description: 'Explore infinite worlds and build everything from the simplest of homes to the grandest of castles.', platforms: ['PC', 'Mobile', 'Console'], genres: ['Sandbox'], image: 'https://picsum.photos/seed/minecraft/600/400', rating: 90, releaseDate: '2011-11-18', isChildFriendly: true }
        ];
        if (pageNum === 1) {
  ...
  const searchGames = useCallback(async (queryOverride?: string, pageNum = 1) => {
    ...
    try {
      if (isAdded) {
        ...
        if (userDoc.exists()) {
          ...
          const itemToRemove = currentWishlist.find((item: any) => item.id === game.id);
          if (itemToRemove) {
    ...
    <div className="mx-auto max-w-7xl px-6 py-12">
      {apiKeyMissing && (
        <div className="mb-8 p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/30 text-yellow-500 text-sm font-bold uppercase tracking-widest text-center">
          ...
          <span className="block text-[10px] mt-1 opacity-60">Please configure RAWG_API_KEY in settings for real game data.</span>
        </div>
      )}

      <div className="mb-16 flex flex-col md:flex-row items-center justify-between gap-6">
        <h1 className="font-display text-6xl font-bold text-plaeen-green uppercase tracking-tighter">
          Create New Session
        </h1>
        <Button 
          variant="outline" 
      ...
      {recommendation && !searchQuery && (
        <div className="mb-20 relative h-[500px] rounded-[2.5rem] overflow-hidden group">
          ...
          <div className="absolute inset-0 flex flex-col justify-center p-12 md:p-20">
            <div className="text-plaeen-green text-xs font-bold uppercase tracking-[0.4em] mb-4">Our Recommendation</div>
            <h2 className="text-6xl md:text-8xl font-bold text-white uppercase tracking-tighter mb-8 max-w-2xl">
              {recommendation.name}
            </h2>
            <div className="flex flex-wrap gap-4">
              <Button 
                onClick={() => teamId ? createSession(recommendation) : fetchGameDetails(recommendation)}
              ...
              <Button 
                variant="outline"
                onClick={() => window.open(`https://rawg.io/games/${recommendation.slug || recommendation.id}`, '_blank')}
      ...
      <div className="mx-auto max-w-4xl mb-12">
        <form onSubmit={(e) => { e.preventDefault(); setPage(1); searchGames(); }} className="relative group mb-8">
      ...
      {loading && page === 1 ? (
        <div className="flex flex-col items-center justify-center py-20 gap-6">
          ...
          <p className="text-plaeen-green font-bold uppercase tracking-[0.3em] animate-pulse">Scanning Database...</p>
        </div>
      ) : (
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {games.map((game) => (
            <Card key={game.id} className="group relative overflow-hidden bg-plaeen-purple/20 border-white/5 p-0 hover:border-plaeen-green/30 transition-all duration-500">
              ...
              <div className="p-6">
                ...
                <div className="space-y-2 mb-6">
                  <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                    ...
                    <span className="text-white/60">{game.releaseDate !== 'TBA' ? format(new Date(game.releaseDate), 'd MMMM yyyy') : 'TBA'}</span>
                  </div>
                  <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                    <span className="text-white/20">Genre</span>
                    <span className="text-white/60 truncate max-w-[150px]">{game.genres.join(', ')}</span>
                  </div>
                  <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                ...
                <div className="flex gap-2">
                  {isGameApproved(game.id) ? (
                    <Button 
                      onClick={() => teamId ? createSession(game) : fetchGameDetails(game)}
                  ...
                  >
                    <Heart size={18} className={wishlistIds.includes(game.id) ? "fill-current" : ""} />
                  </button>
                  <button 
                    onClick={() => window.open(`https://rawg.io/games/${game.slug || game.id}`, '_blank')}
          ...
          <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto custom-scrollbar bg-plaeen-dark border-plaeen-green/30 p-0 shadow-[0_0_50px_rgba(118,233,0,0.1)]">
            ...
            <div className="p-12">
              <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-12">
                <div>
                  <h2 className="text-6xl font-bold text-white uppercase tracking-tighter mb-4">{selectedGame.name}</h2>
                  <div className="flex flex-wrap gap-3">
                    ...
                    <span className="px-4 py-1 rounded-full bg-white/5 border border-white/10 text-white/40 text-[10px] font-bold uppercase tracking-widest">
                      Released: {selectedGame.releaseDate !== 'TBA' ? format(new Date(selectedGame.releaseDate), 'd MMMM yyyy') : 'TBA'}
                    </span>
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-5xl font-bold text-plaeen-green mb-1">{selectedGame.rating}%</div>
                  <div className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em]">Metascore</div>
                </div>
              </div>

              <div className="grid md:grid-cols-3 gap-12">
                <div className="md:col-span-2">
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.4em] text-plaeen-green mb-6">About the game</h3>
                  {loadingDetails ? (
                    <div className="flex items-center gap-4 text-plaeen-green animate-pulse">
                      <Sparkles size={20} />
                      <span className="font-bold uppercase tracking-widest">Fetching full intel...</span>
                    </div>
                  ) : (
                ...
                <div>
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.4em] text-plaeen-green mb-6">Platforms</h3>
                  <div className="grid grid-cols-2 gap-4">
                    {selectedGame.platforms.map(p => (
                      <div key={p} className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
                  ...
                  <div className="mt-12 space-y-4">
                    {isGameApproved(selectedGame.id) ? (
                      <Button 
                        onClick={() => teamId ? createSession(selectedGame) : toggleWishlist(selectedGame)}
                        className={cn(
                          "w-full py-6 font-bold uppercase tracking-widest gap-3",
                          teamId ? "bg-plaeen-green text-black" : (wishlistIds.includes(selectedGame.id) ? "bg-white/5 border-white/10 text-white/40" : "bg-plaeen-green text-black")
                        )}
                      >
                        {teamId ? (
                    ...
                    <Button 
                      variant="outline"
                      onClick={() => window.open(`https://rawg.io/games/${selectedGame.slug || selectedGame.id}`, '_blank')}
      ...
      {!loading && games.length === 0 && searchQuery && (
        <div className="text-center py-20">
          ...
          <p className="text-xl font-bold text-white/20 uppercase tracking-widest">No games found in this sector</p>
        </div>
      )}
    </div>
  );
