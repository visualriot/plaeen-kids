...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Info, Save, RotateCcw } from 'lucide-react';
...
export const CalendarPage = () => {
  ...
  const [selectionMode, setSelectionMode] = useState<AvailabilityType>('available');
  ...
  const toggleDay = (dayKey: string) => {
    ...
    const allSelected = dayHours.every(key => newAvailability[key] === selectionMode);
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-12">
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="font-display text-7xl font-bold text-plaeen-green uppercase tracking-tighter">
            Your Calendar <span className="text-white/20 text-4xl ml-2">?</span>
          </h1>
          <p className="mt-2 text-white/40 font-bold tracking-widest uppercase">Set your gaming availability</p>
        </div>
        <Button variant="outline" className="border-white/20 text-white hover:bg-white/10 gap-2 font-bold uppercase tracking-widest">
          Import calendars
        </Button>
      </div>
      ...
      <Card className="bg-plaeen-purple/20 border-none p-8 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-12 relative z-10">
          <div className="flex items-center gap-6">
            <button onClick={prevWeek} className="text-plaeen-green hover:scale-110 transition-transform">
              <ChevronLeft size={32} />
            </button>
            <span className="text-3xl font-bold text-white tracking-tight">
              {format(days[0], 'dd')}-{format(days[6], 'dd.MM')}
            </span>
            <button onClick={nextWeek} className="text-plaeen-green hover:scale-110 transition-transform">
          ...
          <div className="flex items-center gap-6 relative">
            ...
            >
              ...
              <CalendarIcon size={28} className="text-plaeen-green group-hover:rotate-12 transition-transform" />
            </div>
            <button className="text-plaeen-green hover:scale-110 transition-transform">
              <ChevronRight size={32} />
            </button>
              ...
              <div className="absolute top-full right-0 mt-4 z-50">
                <Card className="bg-plaeen-dark/95 border-white/10 p-6 shadow-2xl backdrop-blur-xl">
                  <div className="grid grid-cols-7 gap-1">
                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map(d => (
                      <div key={d} className="text-center text-[10px] font-bold text-white/40 mb-2">{d}</div>
                    ))}
                    {eachDayOfInterval({
                      start: startOfMonth(currentDate),
                      end: endOfMonth(currentDate)
                    }).map((day, i) => {
                      const isCurrentWeek = isSameDay(startOfWeek(day, { weekStartsOn: 1 }), startOfWeek(currentDate, { weekStartsOn: 1 }));
                      return (
                        <button
                          key={i}
                          onClick={() => {
                          ...
                          className={cn(
                            ...
                            isSameDay(day, currentDate) ? "bg-plaeen-green text-black" : 
                            isCurrentWeek ? "bg-white/10 text-white" : "text-white/40 hover:bg-white/5"
                          )}
                        >
                          {format(day, 'd')}
        ...
        <div className="overflow-x-auto custom-scrollbar pb-4">
          <div className="min-w-[900px]">
            <div className="grid grid-cols-[160px_repeat(24,1fr)] gap-3 mb-6">
              <div />
              {hours.map(h => (
                <div key={h} className="text-center text-xs font-bold text-white/20">{h + 1}</div>
              ))}
            </div>
            ...
            {days.map((day) => {
              ...
              const isDayFull = hours.every(h => availability[`${dayKey}_${h}`] === selectionMode);

              return (
                <div key={dayKey} className="grid grid-cols-[160px_repeat(24,1fr)] gap-3 mb-3 items-center">
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={() => toggleDay(dayKey)}
                      className={cn(
                        "h-5 w-5 rounded border transition-all",
                        isDayFull ? "bg-plaeen-green border-plaeen-green" : "border-white/20 bg-white/5 hover:border-plaeen-green/50"
                      )}
                    />
                    <span className="text-sm font-bold text-white/40 tracking-[0.2em]">{dayName}</span>
                  </div>
                  {hours.map(hour => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          status === 'available' ? "bg-plaeen-green shadow-[0_0_15px_rgba(118,233,0,0.3)]" :
                          status === 'unavailable' ? "bg-red-500/20 border-red-500/30" :
                          status === 'once' ? "bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)]" :
        ...
        <div className="mt-12 pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex flex-wrap gap-8">
            <button 
              ...
              className={cn(
                ...
                selectionMode === 'available' ? "opacity-100 scale-105" : "opacity-40 hover:opacity-60"
              )}
            >
              <div className="h-6 w-6 rounded-lg bg-plaeen-green shadow-[0_0_10px_rgba(118,233,0,0.5)]" />
              <span className="text-sm font-bold text-white tracking-widest uppercase">Always available</span>
            </button>
            <button 
              onClick={() => setSelectionMode('unavailable')}
              className={cn(
                "flex items-center gap-3 transition-all",
                selectionMode === 'unavailable' ? "opacity-100 scale-105" : "opacity-40 hover:opacity-60"
              )}
            >
              <div className="h-6 w-6 rounded-lg bg-red-500/50 border border-red-500" />
              <span className="text-sm font-bold text-white tracking-widest uppercase">Always not available</span>
            </button>
            <button 
              onClick={() => setSelectionMode('once')}
              className={cn(
                "flex items-center gap-3 transition-all",
                selectionMode === 'once' ? "opacity-100 scale-105" : "opacity-40 hover:opacity-60"
              )}
            >
              <div className="h-6 w-6 rounded-lg bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
              <span className="text-sm font-bold text-white tracking-widest uppercase">Available once only</span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Button onClick={handleSave} className="gap-2 px-8 py-4 text-sm font-bold uppercase tracking-widest">
      ...
      <p className="mt-6 text-center text-white/20 text-sm">Click on chosen slot to change availability</p>
    </div>
  );
};
