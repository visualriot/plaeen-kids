  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      async (error) => {
        ...
        console.warn(
          "Approval listener failed, attempting fallback query...",
          error,
        );
  ...
  const handleDecision = async (action: "forgive" | "extract" | "ban") => {
    ...
    try {
      ...
      if (action === "forgive") {
        ...
        const todayAdjSum = (kid.screenTime?.todayAdjustments || []).reduce(
          (acc: number, adj: any) =>
            acc + (adj.type === "reward" ? adj.minutes : -adj.minutes),
          0,
        );
        const currentAllowance =
          (kid.screenTime?.dailyAllowance || 60) + todayAdjSum;

        await updateDoc(kidRef, { "screenTime.usedToday": currentAllowance });
        decisionData.message =
          "Your overtime was forgiven! No penalty applied.";
        ...
        if (isThisWeek)
          updates["screenTime.weeklyAdjustments"] = increment(-amount);
        if (isThisMonth)
          updates["screenTime.monthlyAdjustments"] = increment(-amount);
      ...
      if (approval.notificationId) {
        try {
          await updateDoc(
            doc(db, "notifications", approval.notificationId),
            updatePayload,
          );
          notifUpdated = true;
        } catch (e) {
      ...
      if (!notifUpdated && approvalId && parentProfile) {
        try {
          ...
          if (!notifSnap.empty) {
            await updateDoc(
              doc(db, "notifications", approvalId),
              updatePayload,
            );
            notifUpdated = true;
          }
      ...
      if (!notifUpdated && parentProfile) {
        ...
        if (!parentNotifSnap.empty) {
          await updateDoc(
            doc(db, "notifications", parentNotifSnap.docs[0].id),
            updatePayload,
          );
          notifUpdated = true;
        }
          ...
          <Card className="lg:col-span-1 bg-red-500/5 border-red-500/20 p-8 flex flex-col justify-center items-center text-center space-y-4">
            ...
            <div>
              ...
              <h2 className="text-6xl font-bold text-white">
                {approval.data.overtimeMinutes}m
              </h2>
              <p className="text-white/40 text-xs mt-2">
                Exceeded daily allowance
              </p>
            </div>
          </Card>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) =>
                          setDeductionAmount(parseInt(e.target.value) || 0)
                        }
        ...
        <div className="pt-12 border-t border-white/5 flex justify-center">
          <button
            onClick={async () => {
              if (
                window.confirm(
                  "Are you sure you want to dismiss this alert without taking any action?",
                )
              ) {
                await deleteDoc(doc(db, "approvals", approval.id));
