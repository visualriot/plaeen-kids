          ...
          {[1, 2, 3].map((s) => (
            <div 
              ...
              className={cn(
                ...
                step >= s ? "bg-plaeen-green shadow-[0_0_10px_rgba(118,233,0,0.5)]" : "bg-white/10"
              )}
            />
          ))}
        </div>
        ...
        {step === 1 && (
          <Card className="p-12 bg-plaeen-purple/20 border-white/10 backdrop-blur-2xl">
            <div className="text-center mb-12">
              ...
              <h1 className="text-5xl font-bold text-white uppercase tracking-tighter mb-4">Add Your <span className="text-plaeen-green">Kids</span></h1>
              <p className="text-white/40 font-bold tracking-[0.3em] text-xs">You can always adjust these settings later in your dashboard.</p>
            </div>

            <div className="grid lg:grid-cols-2 gap-12 mb-12">
              <div className="space-y-8">
                <div className="space-y-4">
                  <label className="block text-[10px] font-bold uppercase tracking-[0.4em] text-plaeen-green">Kid's Name</label>
                  <input 
                    type="text"
                ...
                <div className="space-y-4">
                  <label className="block text-[10px] font-bold uppercase tracking-[0.4em] text-plaeen-green">Unique Username</label>
                  <input 
                    type="text"
                ...
                <div className="flex gap-6">
                  ...
                  <div className="space-y-4 flex-1">
                    <label className="block text-[10px] font-bold uppercase tracking-[0.4em] text-plaeen-green">Allowance (Min)</label>
                    <div className="flex gap-2">
                      <input 
                        type="number"
                        value={currentKid.dailyAllowance}
                        onChange={e => setCurrentKid({...currentKid, dailyAllowance: parseInt(e.target.value)})}
                      ...
                      <select 
                        ...
                        onChange={e => setCurrentKid({...currentKid, allowanceType: e.target.value as any})}
                      ...
                      >
                        <option value="daily" className="bg-plaeen-dark text-white py-4">Daily</option>
                        <option value="weekly" className="bg-plaeen-dark text-white py-4">Weekly</option>
                        <option value="monthly" className="bg-plaeen-dark text-white py-4">Monthly</option>
                      </select>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <label className="block text-[10px] font-bold uppercase tracking-[0.4em] text-plaeen-green">Restricted Days (No Play)</label>
                ...
                <div className="flex items-center justify-between p-6 rounded-2xl bg-white/5 border border-white/5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs font-bold text-white uppercase tracking-widest">Parental Control</p>
                      <p className="text-[10px] text-white/40 uppercase tracking-widest mt-1">Restricted content & approvals</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => setCurrentKid({...currentKid, parentalControl: !currentKid.parentalControl})}
                    className={cn(
                      "h-6 w-12 rounded-full transition-all relative",
                      currentKid.parentalControl ? "bg-plaeen-green" : "bg-white/10"
                    )}
                  >
                    <div className={cn(
                ...
                <Button onClick={handleAddKid} variant="outline" className="w-full py-6 border-plaeen-green/30 text-plaeen-green hover:bg-plaeen-green hover:text-black">
                  <Plus size={20} className="mr-2" /> Add to Family
                </Button>
              </div>

              <div className="space-y-8">
                <h3 className="text-[10px] font-bold uppercase tracking-[0.4em] text-white/40">Family ({kids.length})</h3>
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-4 custom-scrollbar">
                  {kids.map((k, i) => (
                    <div key={i} className="flex items-center justify-between p-6 rounded-2xl bg-white/5 border border-white/10 group hover:border-plaeen-green/30 transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-white uppercase tracking-widest">{k.name}</p>
                          <p className="text-[10px] text-white/40 uppercase tracking-widest">@{k.username}</p>
                        </div>
                      </div>
                      <button onClick={() => removeKid(i)} className="text-white/20 hover:text-red-500 transition-colors">
                        <X size={20} />
                      </button>
                    </div>
                  ))}
                  {kids.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center py-20 text-center opacity-20">
                      ...
                      <p className="text-xs font-bold uppercase tracking-widest">Your family is empty</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        {step === 2 && (
          <Card className="p-12 bg-plaeen-purple/20 border-white/10 backdrop-blur-2xl max-w-md mx-auto">
            <div className="text-center mb-12">
              ...
              <h1 className="text-5xl font-bold text-white uppercase tracking-tighter mb-4">Secure <span className="text-plaeen-green">Access</span></h1>
              <p className="text-white/40 font-bold uppercase tracking-[0.3em] text-xs">Set your 4-digit guardian PIN</p>
            </div>
            ...
            <div className="space-y-8">
                ...
                {[0, 1, 2, 3].map((i) => (
                  <div 
                    ...
                    className={cn(
                      ...
                      pin.length > i ? "border-plaeen-green text-plaeen-green bg-plaeen-green/10" : "border-white/10 text-white/20"
                    )}
                  >
                    {pin[i] ? '•' : ''}
                  </div>
              ...
              <div className="flex gap-4 pt-8">
                <Button variant="ghost" onClick={() => setStep(1)} className="flex-1 border border-white/10">
                  Back
                </Button>
                <Button 
