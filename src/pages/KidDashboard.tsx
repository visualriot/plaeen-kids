...
export const KidDashboard = () => {
  ...
  // Only load kid data if user is confirmed to be a kid
  useEffect(() {
    if (!user || userRole !== "kid") return;

    const unsubscribeKid = onSnapshot(doc(db, "users", user.uid), (docSnapshot) => {
      if (docSnapshot.exists()) {
        setKidData({ uid: docSnapshot.id, ...docSnapshot.data() } as KidData);
      }
    });
    ...
    if (isSessionActive && sessionStartTime) {
      interval = setInterval(async () => {
        ...
        if (diffMinutes > elapsedMinutes) {
          ...
          // Check for warnings
          const remaining =
            (kidData?.screenTime.dailyAllowance || 0) -
            (kidData?.screenTime.usedToday || 0);
  ...
  if (userRole !== "kid") {
    return (
      <div className="flex h-screen items-center justify-center">
        Access denied. Kid account required.
      </div>
    );
  }
  ...
  const dailyAllowance = kidData.screenTime?.dailyAllowance || 120;
  const usedToday = kidData.screenTime?.usedToday || 0;
  const remainingTime = dailyAllowance - usedToday;
  const timePercent = (remainingTime / dailyAllowance) * 100;
    ...
    <div className="mx-auto max-w-7xl px-6 py-12">
      <div className="mb-12">
        <h1 className="text-6xl font-bold text-white uppercase tracking-tighter drop-shadow-[0_0_30px_rgba(118,233,0,0.3)]">
          Welcome,{" "}
          <span className="text-plaeen-green">{kidData.displayName}</span>
        </h1>
          ...
          <Card className="bg-plaeen-purple/20 border-plaeen-green/30 p-10 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <div className="flex justify-between mt-4 text-[10px] font-bold uppercase tracking-widest text-white/20">
                <span>{usedToday}m Used</span>
                <span>{dailyAllowance}m Limit</span>
              </div>
            </div>
          </Card>
        ...
        <div className="space-y-12">
            ...
            <Card className="bg-white/5 border-white/10 p-6 relative overflow-hidden">
              ...
              {isSessionActive ? (
                <div className="animate-pulse">
                  ...
                  <p className="text-2xl font-bold text-white uppercase tracking-tight mb-4">
                    Playing Now
                  </p>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-white/40 font-bold uppercase tracking-widest text-[10px]">
                    <Clock size={12} />{" "}
                    {format(nextSession.startTime.toDate(), "HH:mm")}
                  </div>
                </div>
              ) : (
                <div className="text-center py-4">
                  ...
                  <Link to="/calendar">
                    <Button
                      variant="outline"
                      size="sm"
                      className="border-white/10 text-white/40 hover:text-plaeen-green text-[8px] font-bold uppercase tracking-widest"
                    >
                      Create One
                    </Button>
                  </Link>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Link
                to="/search"
                className="aspect-square rounded-xl border border-dashed border-white/10 flex items-center justify-center hover:border-plaeen-green/50 transition-all group"
              >
                <Plus
                  size={24}
                  className="text-white/10 group-hover:text-plaeen-green"
                />
              </Link>
            </div>
          </section>
