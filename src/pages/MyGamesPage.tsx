...
export const MyGamesPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'wishlist' | 'approved' | 'rejected'>('wishlist');
  ...
  const approvedGames = wishlist.filter(g => activeKid?.allowedGames?.includes(g.id));
  const pendingGames = approvals.filter(a => a.status === 'pending');
  const rejectedGames = approvals.filter(a => a.status === 'denied');

  if (loading) return <div className="flex h-[60vh] items-center justify-center text-white">Loading...</div>;
    ...
    <div className="mx-auto max-w-7xl px-6 py-12">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 gap-6">
        <div>
          ...
          <p className="text-white/40 font-bold uppercase tracking-[0.4em] text-xs mt-2">Manage your library & wishlist</p>
        </div>
        <Button 
      ...
      <div className="flex gap-4 mb-12 border-b border-white/10 pb-4">
        ...
        >
          Rejected ({rejectedGames.length}) {showRejected ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
        </button>
      </div>
          ...
          >
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {rejectedGames.map(game => (
                <Card key={game.id} className="bg-red-500/5 border-red-500/20 p-4 opacity-60">
                  <div className="flex items-center gap-4">
                    <img src={game.data.image} alt={game.data.gameName} className="h-12 w-12 rounded-lg object-cover grayscale" />
                    <div>
                      <h3 className="text-sm font-bold text-white uppercase">{game.data.gameName}</h3>
                      <p className="text-[8px] text-red-400 uppercase font-bold">Denied by Parent</p>
                    </div>
                  </div>
                </Card>
              ))}
              {rejectedGames.length === 0 && <p className="text-white/20 text-xs uppercase font-bold">No rejected games</p>}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
        {(activeTab === 'wishlist' ? wishlist : approvedGames).map((game) => (
          <Card key={game.id} className="p-0 overflow-hidden border-white/10 bg-white/5 group hover:border-plaeen-green/30 transition-all duration-500">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-end p-6">
                <div className="flex gap-2 mb-4 opacity-0 group-hover:opacity-100 transition-opacity translate-y-4 group-hover:translate-y-0 duration-300">
                  <Button 
                    ...
                    onClick={() => navigate(`/search?q=${encodeURIComponent(game.name)}`)}
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="relative group/menu">
                  ...
                  <div className="absolute bottom-full left-0 mb-2 w-48 bg-plaeen-dark border border-white/10 rounded-xl p-2 hidden group-hover/menu:block z-20 shadow-2xl">
                    <p className="text-[8px] font-bold text-white/20 uppercase tracking-widest p-2 border-b border-white/5 mb-2">Select Team</p>
                    {teams.map(team => (
                      <button 
                        key={team.id}
                    ...
                    {teams.length === 0 && <p className="text-[8px] text-white/20 p-2">No teams joined</p>}
                  </div>
                </div>
                ...
                <div className="relative group/menu">
                  ...
                  <div className="absolute bottom-full right-0 mb-2 w-48 bg-plaeen-dark border border-white/10 rounded-xl p-2 hidden group-hover/menu:block z-20 shadow-2xl">
                    <p className="text-[8px] font-bold text-white/20 uppercase tracking-widest p-2 border-b border-white/5 mb-2">Select Friend</p>
                    {friends.map(friend => (
                      <button 
                        key={friend.id}
                    ...
                    {friends.length === 0 && <p className="text-[8px] text-white/20 p-2">No friends found</p>}
                  </div>
                </div>
              </div>
            </div>
      ...
      {(activeTab === 'wishlist' ? wishlist : approvedGames).length === 0 && (
        <Card className="text-center py-24 bg-white/5 border-dashed border-white/10 rounded-[2.5rem]">
          ...
          <h2 className="text-3xl font-bold text-white uppercase tracking-tighter mb-4">No games found here</h2>
