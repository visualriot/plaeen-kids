...
export const KidCalendarPage = () => {
  ...
  useEffect(() => {
    ...
    const qGroups = query(
      ...
      where(isParentViewer ? 'parentIds' : 'members', 'array-contains', isParentViewer ? activeKid.parentId : activeKid.uid)
    );
    ...
    const unsubGroups = onSnapshot(qGroups, (snapshot) => {
      try {
        ...
        const filteredDocs = isParentViewer 
          ? snapshot.docs.filter(d => d.data().members?.includes(activeKid.uid))
          : snapshot.docs;
    ...
    const fetchAllSessions = async () => {
      try {
        const fetchPromises = groupIds.map(async (gid) => {
          ...
          return snap.docs.map(d => ({ id: d.id, groupId: gid, ...d.data() }));
        });
  ...
  const toggleDay = async (day: Date) => {
    ...
    setIsUpdating(true);
    await updateDoc(doc(db, 'users', activeKid.uid), { availability: newAvailability });
    setIsUpdating(false);
  };
    ...
    <div className="mx-auto max-w-7xl px-6 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16">
        <div>
          ...
          <p className="text-white/40 font-bold uppercase tracking-[0.4em] text-xs mt-2">Manage your availability and team sessions</p>
        </div>
      ...
      <Card className="bg-white/5 border-white/10 p-4 lg:p-10 overflow-hidden relative">
        <div className="w-full">
          ...
          {days.map((day) => {
            ...
            return (
              <div key={dayKey} className="grid grid-cols-[80px_repeat(24,1fr)] gap-0.5 mb-0.5 md:mb-3 md:gap-3 items-center">
                <button 
                  onClick={() => toggleDay(day)}
                  className="text-left text-[8px] md:text-[10px] font-bold text-white/40 hover:text-white transition-colors uppercase tracking-tight md:tracking-widest group flex items-center gap-1"
                >
                  <span className="w-12 md:w-16">{format(day, 'EEE d')}</span>
                    <RotateCcw size={12} className="opacity-0 group-hover:opacity-100 transition-opacity text-plaeen-green" />
                  </button>
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          session?.status === 'scheduled' ? "bg-plaeen-green shadow-[0_0_20px_rgba(118,233,0,0.6)] z-20 border-white/60" :
                          session?.status === 'proposed' ? "bg-amber-400 border-2 border-white/40 shadow-[0_0_15px_rgba(251,191,36,0.3)] z-10" :
                          state === 'recurring' ? "bg-plaeen-green/40 shadow-[0_0_10px_rgba(118,233,0,0.1)] border-plaeen-green/20" :
                          state === 'once' ? "bg-plaeen-green/20 border-plaeen-green shadow-[0_0_10px_rgba(118,233,0,0.1)]" :
                      ...
                      >
                        {session && (
                          <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-plaeen-dark border border-white/10 px-3 py-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all scale-95 group-hover:scale-100 whitespace-nowrap z-50 pointer-events-none shadow-2xl">
                            <p className="text-[10px] font-bold text-plaeen-purple uppercase tracking-[0.2em] mb-1">Team Session</p>
                            <p className="text-xs font-bold text-white uppercase">{session.gameName || 'Gaming'}</p>
                          </div>
                        )}
                      </div>
                    );
        ...
        <div className="mt-12 pt-8 border-t border-white/5 flex flex-wrap gap-x-12 gap-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-white uppercase tracking-widest">Available (Recurring)</p>
              <p className="text-[10px] text-white/20 font-bold uppercase tracking-widest">Repeats every week</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-white uppercase tracking-widest">Available (Once)</p>
              <p className="text-[10px] text-white/20 font-bold uppercase tracking-widest">Only for this date</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-white uppercase tracking-widest">Scheduled Session</p>
              <p className="text-[10px] text-white/20 font-bold uppercase tracking-widest">Team gaming event</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-white uppercase tracking-widest">Unavailable</p>
              <p className="text-[10px] text-white/20 font-bold uppercase tracking-widest">Default state</p>
            </div>
          </div>
        </div>
