...
import { Shield, Lock, Check, X, ChevronLeft, Calendar, Eye, EyeOff, Trash2, AlertTriangle, LogOut } from 'lucide-react';
...
export const ParentSettingsPage = () => {
  ...
  const handlePermanentDeletion = async () => {
    ...
    try {
      ...
      const parentNotifSnap = await getDocs(query(collection(db, 'notifications'), where('parentId', '==', parentProfile.uid)));
      parentNotifSnap.forEach(d => batch.delete(d.ref));
      
      // Also delete notifications explicitly assigned to the parent's UID
      const ownNotifSnap = await getDocs(query(collection(db, 'notifications'), where('userId', '==', parentProfile.uid)));
      ownNotifSnap.forEach(d => batch.delete(d.ref));

      // 2. Delete approvals
      // Querying by parentId is guaranteed to work for parents
      const approvalParentSnap = await getDocs(query(collection(db, 'approvals'), where('parentId', '==', parentProfile.uid)));
      approvalParentSnap.forEach(d => batch.delete(d.ref));

      // 3. Delete sessions
      // Sessions also have parentId
      const sessionParentSnap = await getDocs(query(collection(db, 'sessions'), where('parentId', '==', parentProfile.uid)));
      ...
      if (kidIds.length > 0) {
        ...
        for (const chunk of kidChunks) {
          const sessionKidSnap = await getDocs(query(collection(db, 'sessions'), where('childId', 'in', chunk)));
          sessionKidSnap.forEach(d => batch.delete(d.ref));
        }
      }

      // 4. Delete friends requests
      const frFromParentSnap = await getDocs(query(collection(db, 'friendRequests'), where('fromParentId', '==', parentProfile.uid)));
      frFromParentSnap.forEach(d => batch.delete(d.ref));
      const frToParentSnap = await getDocs(query(collection(db, 'friendRequests'), where('toParentId', '==', parentProfile.uid)));
      frToParentSnap.forEach(d => batch.delete(d.ref));
      
      // Also delete based on direct UID just in case (legacy or parent's own requests)
      const frOwnFromSnap = await getDocs(query(collection(db, 'friendRequests'), where('fromId', '==', parentProfile.uid)));
      frOwnFromSnap.forEach(d => batch.delete(d.ref));
      const frOwnToSnap = await getDocs(query(collection(db, 'friendRequests'), where('toId', '==', parentProfile.uid)));
      ...
      const groupsOwnedSnap = await getDocs(query(collection(db, 'groups'), where('ownerId', 'in', uidsToDelete)));
      ...
      try {
        ...
        console.warn('Auth deletion failed - usually requires fresh login. Signing out instead.', authErr);
        await auth.signOut();
      }
      ...
      alert('Account deletion failed. This usually happens due to security permissions or a lost connection. Your data might still be present.');
    }
  };
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card className="bg-red-500/5 border-red-500/20 p-8">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
                <h3 className="text-xl font-bold text-white uppercase tracking-tight">Delete Account</h3>
              ...
              <Button 
                ...
                variant="outline"
                className="border-red-500/30 text-red-500 hover:bg-red-500/10 font-bold uppercase tracking-widest text-[10px] py-4 px-8"
              >
                Delete Everything
              </Button>
            </div>
              ...
              <Card className="bg-plaeen-dark border-red-500/30 p-10 text-center overflow-hidden relative">
                ...
                {deleteConfirmStep === 1 && (
                  <div className="space-y-8">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-white uppercase tracking-tighter mb-4">First Warning</h2>
                      <p className="text-white/40 text-sm font-bold uppercase tracking-widest leading-relaxed">
                        Are you absolutely sure? This will delete your profile and <span className="text-red-500">all {parentProfile.linkedKids?.length || 0} kid accounts</span> associated with you.
                      </p>
                    </div>
                    <div className="flex gap-4">
                      <Button 
                        onClick={() => setDeleteConfirmStep(2)}
                        className="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold uppercase tracking-widest py-6"
                      >
                      ...
                      <Button 
                        ...
                        className="flex-1 text-white/40 hover:text-white font-bold uppercase tracking-widest py-6"
                      >
                        Cancel
                      </Button>
                    </div>
                ...
                {deleteConfirmStep === 2 && (
                  <div className="space-y-8">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-white uppercase tracking-tighter mb-4">Final Confirmation</h2>
                      <p className="text-white/60 text-xs font-bold uppercase tracking-widest leading-relaxed mb-4">
                        This action is <span className="text-white">permanent</span> and cannot be undone. All play history, approvals, and shared team data will be wiped.
                      </p>
                    ...
                    <div className="flex flex-col gap-4">
                      <Button 
                        ...
                        disabled={isDeleting}
                        className="w-full bg-white text-black hover:bg-red-500 hover:text-white font-bold uppercase tracking-widest py-10 transition-all duration-500 text-lg shadow-[0_0_50px_rgba(255,255,255,0.1)]"
                      >
                        {isDeleting ? 'Erasing Data...' : 'Confirm Permanent Deletion'}
                      </Button>
                      <Button 
                        variant="ghost"
                        onClick={() => setIsDeleteDialogOpen(false)}
                        disabled={isDeleting}
                        className="w-full text-white/40 hover:text-white font-bold uppercase tracking-widest py-4"
                      >
                        Abort
                      </Button>
                    </div>
