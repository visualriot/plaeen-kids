...
import { doc, onSnapshot, updateDoc, arrayUnion, arrayRemove, getDoc, collection, query, where, getDocs, deleteDoc, addDoc, serverTimestamp, writeBatch, Timestamp } from 'firebase/firestore';
import { useAuthState } from 'react-firebase-hooks/auth';
import { useProfile } from '@/contexts/ProfileContext';
import { Button } from '@/components/Button';
import { Card } from '@/components/Card';
import { ChevronLeft, Trash2, Shield, UserPlus, Image as ImageIcon, Check, X, LogOut, Settings, Plus, Sparkles } from 'lucide-react';
...
export const TeamSettingsPage = () => {
  ...
  const [confirmModal, setConfirmModal] = useState<{ type: 'leave' | 'delete', targetId?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'groups', teamId), async (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.id ? { id: docSnap.id, ...docSnap.data() } as Team : null;
        if (data) {
          // Check if user is still a member
          if (!data.members.includes(activeUid) && !(data.pendingMembers || []).includes(activeUid)) {
          ...
          const membersQuery = query(collection(db, 'users_public'), where('uid', 'in', data.members));
  ...
  const handleToggleAdmin = async (memberId: string, isCurrentlyAdmin: boolean) => {
    ...
    try {
      ...
      showFeedback(isCurrentlyAdmin ? 'Admin rights removed' : 'New admin assigned', 'info');
    } catch (err) {
      console.error(err);
    }
  };
  ...
  if (loading || !team) return (
    <div className="flex h-screen items-center justify-center bg-plaeen-dark">
      <div className="text-white font-bold uppercase tracking-widest animate-pulse">Scanning Data...</div>
    </div>
  );

  const hasAdminRights = (team.adminIds || []).includes(activeUid || '') || 
          ...
          <section>
            ...
            <Card className="bg-white/5 border-white/5 p-6 space-y-4">
              {sortedMembers.map(member => {
                const isMemberAdmin = (team.adminIds || []).includes(member.uid);
                ...
                return (
                  <div key={member.uid} className={cn(
                    ...
                    isMemberAdmin ? "border-plaeen-green/30" : "border-white/5 hover:border-white/20"
                  )}>
                    <div className="flex items-center gap-4">
                      <div className={cn(
                        "h-12 w-12 rounded-xl p-1 bg-plaeen-dark relative",
                        isMemberAdmin ? "border-2 border-plaeen-green" : "border border-white/10"
                      )}>
                        <img src={member.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${member.uid}`} className="h-full w-full rounded-lg object-cover" />
                        {isMemberAdmin && (
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-bold text-white uppercase">{member.displayName} {isMe && '(You)'}</p>
                          <span className={cn(
                            "text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded",
                            isMemberAdmin ? "bg-plaeen-green/10 text-plaeen-green" : "bg-white/5 text-white/40"
                          )}>
                            {isMemberAdmin ? 'Admin' : 'Member'}
                          </span>
                        </div>
                        <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">@{member.username}</p>
                      </div>
                    </div>
                        ...
                        <>
                          <button
                            ...
                            className={cn(
                              ...
                              isMemberAdmin ? "bg-plaeen-green/20 text-plaeen-green border border-plaeen-green/30" : "bg-white/5 text-white/40 hover:text-white"
                            )}
                          >
            ...
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {team.pendingMembers.map(id => {
                  ...
                  return (
                    <Card key={id} className="bg-white/5 border-white/5 p-4 flex items-center gap-3 opacity-60">
                      <div className="h-8 w-8 rounded-full bg-white/10 overflow-hidden">
                        <img src={friend?.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${id}`} className="h-full w-full object-cover" />
                      </div>
                      <div className="truncate">
                        <p className="text-[10px] font-bold text-white uppercase truncate">{friend?.displayName || 'Scanning...'}</p>
                      </div>
                      {hasAdminRights && (
                        <button 
                          onClick={() => updateDoc(doc(db, 'groups', team.id), { pendingMembers: arrayRemove(id) })}
        ...
        <div className="space-y-12">
          ...
          <section>
            ...
            <Card className="bg-white/5 border-white/5 p-8 space-y-10">
              <div>
                <label className="block text-[8px] font-bold uppercase tracking-[0.3em] text-white/40 mb-4">Team Avatar</label>
                  ...
                  {AVATARS.map((avatar) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedAvatar === avatar ? "border-plaeen-green scale-105 shadow-[0_0_20px_rgba(118,233,0,0.3)]" : "border-white/5 opacity-40 grayscale hover:grayscale-0 hover:opacity-100"
                      )}
                    >
              ...
              <div>
                <label className="block text-[8px] font-bold uppercase tracking-[0.3em] text-white/40 mb-4">Team Callsign</label>
                <input
                  type="text"
                  value={editingName}
          ...
          >
            <Card className="w-full max-w-md bg-plaeen-dark border-plaeen-green/30 p-10">
              <div className="flex justify-between items-center mb-10">
                <h2 className="text-4xl font-bold text-white uppercase tracking-tighter">Invite Players</h2>
                <button onClick={() => setIsAddMemberOpen(false)} className="text-white/40 hover:text-white transition-colors">
                  <X size={32} />
                </button>
              </div>

              <div className="space-y-4 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                {friends.filter(f => !team.members.includes(f.uid) && !(team.pendingMembers || []).includes(f.uid)).map(friend => (
                  <div key={friend.uid} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/10">
                    <div className="flex items-center gap-4 text-left">
                      ...
                      <div>
                        <p className="text-sm font-bold text-white uppercase">{friend.displayName}</p>
                        <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">@{friend.username}</p>
                      </div>
                    </div>
                    <Button size="sm" onClick={() => inviteFriend(friend.uid)} className="h-10 px-6">Invite</Button>
                  </div>
                ))}
                {friends.filter(f => !team.members.includes(f.uid) && !(team.pendingMembers || []).includes(f.uid)).length === 0 && (
                  <div className="py-12 text-center">
                    <p className="text-white/20 font-bold uppercase tracking-widest text-xs">All friends are already in high command</p>
                  </div>
                )}
              </div>
            </Card>
          ...
          >
            <Card className="w-full max-w-md bg-plaeen-dark border-red-500/30 p-10 text-center">
              <div className={cn(
                ...
                confirmModal.type === 'delete' ? "bg-red-500/10" : "bg-amber-500/10"
              )}>
                {confirmModal.type === 'delete' ? <Trash2 size={32} className="text-red-500" /> : <LogOut size={32} className="text-amber-500" />}
              </div>
              <h2 className="text-2xl font-bold text-white uppercase tracking-tighter mb-4">
                {confirmModal.type === 'delete' ? 'Decommission Team?' : 'Leave Sector?'}
              </h2>
              ...
              <div className="flex flex-col gap-4">
                <Button 
                  onClick={() => confirmModal.type === 'delete' ? handleDeleteTeam() : handleLeaveTeam()}
                  className={cn(
                    "w-full py-6 font-bold uppercase tracking-widest",
                    confirmModal.type === 'delete' ? "bg-red-500 hover:bg-red-600 shadow-[0_0_20px_rgba(239,68,68,0.3)]" : "bg-amber-500 hover:bg-amber-600 shadow-[0_0_20px_rgba(245,158,11,0.3)]"
                  )}
                >
                  Confirm Protocol
                </Button>
          ...
          >
            <div className={cn(
              ...
              <div className={cn(
                ...
                feedback.type === 'success' ? "bg-plaeen-green animate-pulse" : "bg-white"
              )} />
