    ...
    try {
      ...
      if (status === "approved") {
        if (request.type === "game") {
          ...
          const currentAllowance =
            kidDoc.data()?.screenTime?.dailyAllowance || 60;
          await updateDoc(doc(db, "users", request.childId), {
            "screenTime.dailyAllowance":
              currentAllowance +
              (request.data.requestedMinutes || request.data.minutes || 30),
          });
        ...
        // Punishment logic
        const penalty =
          penalties[request.id] || request.data.overtimeMinutes || 0;
        const kidDoc = await getDoc(doc(db, "users", request.childId));
        const currentAccumulated =
          kidDoc.data()?.screenTime?.accumulatedTime || 0;
      ...
      if (request.type !== "overtime") {
        ...
        await addDoc(collection(db, "notifications"), {
          ...
          message:
            "Your parent has forgiven your overtime session. No penalty applied!",
          createdAt: serverTimestamp(),
          read: false,
          status: "approved",
        });
                    ...
                    <div>
                      ...
                      <h3 className="text-2xl font-bold text-white uppercase tracking-tight mb-2">
                        {req.type === "friend"
                          ...
                          : req.type === "game"
                            ? `Access to ${req.data.gameName}`
                            : req.type === "time"
                              ? `Extra Time: +${req.data.minutes}m`
                              : req.type === "overtime"
                                ? `Overtime Alert: ${req.data.overtimeMinutes}m`
                                : req.type === "activity"
                                  ? `Activity: ${req.title}`
                                  : `Join Team: ${req.data.teamName}`}
                      </h3>

                      {req.type === "overtime" && (
                        <div className="mt-4 space-y-4">
                          <p className="text-xs text-white/40 font-medium">
                            {req.childName} exceeded their allowance by{" "}
                            {req.data.overtimeMinutes} minutes. Total session
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <input
                              type="number"
                              value={
                                penalties[req.id] ?? req.data.overtimeMinutes
                              }
                              onChange={(e) =>
                                setPenalties((prev) => ({
                                  ...prev,
                                  [req.id]: parseInt(e.target.value) || 0,
                                }))
                              }
