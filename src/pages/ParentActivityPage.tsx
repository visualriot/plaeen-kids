...
export const ParentActivityPage = () => {
  ...
  }, [notifications, sortBy, sortOrder, filterByKid, filterByType, showUnreadOnly]);
  ...
  const groupedNotifications = useMemo(() => {
    ...
    if (sortBy === 'date') {
      ...
      if (sortOrder === 'desc') {
        ...
        if (thisWeek.length > 0) groups.push({ title: 'Last 7 Days', items: thisWeek });
      ...
      // Group by kid
      const kidsInNotifs = Array.from(new Set(filteredAndSortedNotifications.map(n => n.childName))) as string[];
      if (sortOrder === 'asc') {
        kidsInNotifs.sort();
      } else {
      ...
      kidsInNotifs.forEach(kidName => {
        const items = filteredAndSortedNotifications.filter(n => n.childName === kidName);
  ...
  const summaryStats = useMemo(() => {
    ...
    return kids.map(kid => {
      const kidNotifications = notifications.filter(n => n.childId === kid.uid);
      ...
      const totalMinutesPlayedToday = finishedSessionsToday.reduce((acc, n) => acc + (n.duration || 0), 0);
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h2 className="text-[10px] font-bold uppercase tracking-[0.4em] text-plaeen-green flex items-center gap-3">
              <Bell size={16} /> {filterByType === 'time_warning' ? 'Warning History' : 'Recent Notifications'}
              {filterByKid && ` • ${kids.find(k => k.uid === filterByKid)?.displayName}`}
            </h2>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex bg-white/5 rounded-lg p-0.5 border border-white/10">
                ...
                >
                  ...
                  {sortBy === 'date' && <span className="text-[8px]">{sortOrder === 'asc' ? '↑' : '↓'}</span>}
                </button>
                <button 
                ...
                >
                  <User size={12} />
                  <span className="text-[9px] font-bold uppercase tracking-widest">Kid</span>
                  {sortBy === 'kid' && <span className="text-[8px]">{sortOrder === 'asc' ? '↑' : '↓'}</span>}
                </button>
              </div>
              ...
              <div className="relative group">
                ...
                >
                  <option value="all" className="bg-[#0A0514] text-white">All Activity</option>
                  <option value="unread" className="bg-[#0A0514] text-white">Unread Only</option>
                  <option value="warnings" className="bg-[#0A0514] text-white">All Warnings</option>
                  <optgroup label="Filter by Kid" className="bg-[#0A0514] text-white/40 text-[8px] uppercase">
                    {kids.map(kid => (
                      <option key={kid.uid} value={`kid:${kid.uid}`} className="bg-[#0A0514] text-white">{kid.displayName}</option>
                    ))}
                  </optgroup>
                </select>
                <Filter size={10} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-white/40 group-hover:text-plaeen-green transition-colors" />
                <ChevronDown size={10} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-white/40 pointer-events-none group-hover:text-plaeen-green transition-colors" />
              </div>
          ...
          <div className="space-y-12">
                ...
                >
                  ...
                  <ChevronDown 
                    ...
                    className={cn(
                      ...
                      collapsedGroups.has(group.title) ? "-rotate-90" : "rotate-0"
                    )} 
                  />
                </button>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-white/5">
                                      {!notif.read ? (
                                        <div className="flex items-center gap-3">
                                          ...
                                          <span className="text-[8px] font-bold text-red-500 uppercase tracking-widest animate-pulse">Action Required</span>
                                        </div>
                                      ) : (
                                        <p className="text-[10px] font-bold text-white/60 uppercase tracking-widest">
                                          Status: <span className="text-plaeen-green">
                                            {notif.decision?.message || 'No action taken'}
                                          </span>
                                        </p>
                                      )}
                                    </div>
            ...
            {groupedNotifications.length === 0 && (
              <Card className="bg-white/5 border-dashed border-white/10 p-12 text-center">
                <p className="text-white/20 font-bold uppercase tracking-widest">No activity matches your filters</p>
                <button onClick={clearFilters} className="mt-4 text-plaeen-green text-[10px] font-bold uppercase tracking-widest hover:underline">Clear all filters</button>
              </Card>
            )}
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            <Card className="bg-white/5 border-white/10 p-8">
              <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest mb-6">Total Sessions Today</p>
              <div className="space-y-4">
                {summaryStats.map(kid => (
                  <div key={kid.uid} className="flex justify-between items-end border-b border-white/5 pb-4 last:border-0 last:pb-0">
                    <div>
                      <p className="text-xs font-bold text-white uppercase tracking-widest">{kid.displayName}</p>
                      <p className="text-[10px] text-white/40 uppercase tracking-widest">{kid.sessionsToday} Sessions</p>
                    </div>
                    <div className="text-right">
                      <p className="text-2xl font-bold text-plaeen-green">{kid.totalMinutesPlayedToday}m</p>
                      <p className="text-[8px] text-white/20 uppercase tracking-widest">Played Today</p>
                    </div>
                  </div>
                ))}
                {summaryStats.length === 0 && (
                  <p className="text-xs text-white/20 italic">No kids linked yet</p>
                )}
              </div>
            </Card>

            <Card className="bg-white/5 border-white/10 p-8">
              <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest mb-6">Warnings Triggered (30d)</p>
              <div className="space-y-4">
                {summaryStats.map(kid => (
                  <button 
                    key={kid.uid} 
                  ...
                  >
                    <div className="text-left">
                      <p className="text-xs font-bold text-white uppercase tracking-widest group-hover:text-plaeen-green transition-colors">{kid.displayName}</p>
                      <p className="text-[8px] text-white/20 uppercase tracking-widest">Click to view history</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className={`px-3 py-1 rounded-full text-[10px] font-bold ${kid.warningsLast30Days > 5 ? 'bg-red-500 text-white' : kid.warningsLast30Days > 0 ? 'bg-yellow-500 text-black' : 'bg-white/5 text-white/20'}`}>
                        {kid.warningsLast30Days}
                      </div>
                      <ChevronRight size={14} className="text-white/20 group-hover:text-plaeen-green transition-colors" />
                    </div>
                  </button>
                ))}
              </div>
