export const RESERVED_USERNAMES = ['admin', 'support', 'api', 'system', 'null', 'parent', 'guardian'];
...
export const validateUsername = (username: string): { isValid: boolean; error?: string } => {
  ...
  if (!/^[a-z0-9._-]+$/.test(clean)) {
    return { isValid: false, error: 'Only letters, numbers, dots, hyphens, and underscores allowed.' };
  }
  ...
  if (/[._-]{2,}/.test(clean)) {
    return { isValid: false, error: 'Consecutive symbols (.., __, --) are not allowed.' };
  }
